...
# A stub node that records its arguments and fails for one named config, so we
# can assert the loop continues past a failing domain rather than aborting the
# whole fleet.
cat > "$ROOT/bin/node" <<'STUB'
#!/usr/bin/env bash
echo "NODE:$*"
case "$*" in *broken-com*) exit 3 ;; esac
STUB
...
[ "$(grep -c 'NODE:' <<<"$out")" -ge 6 ] && ok "runs send and engage for each config" || bad "expected two node calls per config"
