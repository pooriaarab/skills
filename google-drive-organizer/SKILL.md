...
`analytics/`, `decks/`, `engineering/`, `events/`, `finance/`, `marketing/`, `media/`, `outreach-leads/`, `product/`, `strategy/`, `support/`, `testing/`, `users-data/`, `personal/`, `mozilla/` (org-specific), `_inbox/`, `_archive/`

**Personal Drive:**
`career/` (resumes/, reference-letters/, job-descriptions/, mozilla/), `immigration/` (canada-pr/, work-permit/, family/, dubai/, us-visa/), `university/` (courses/, textbooks/), `beeloud/` or other-startup/ (events/, grants/, partnerships/, community/), `fitness/` (<workout-logs>/), `finance/`, `media/` (recordings/, photos/, brand-assets/), `languages/`, `ai-projects/` (notebooks/, image-gen/), `creative/` (content-planning/, brand-assets/, project-name/), `personal/`, `_archive/`, `_inbox/`
...
| Pattern | Action |
|---------|--------|
| `Feb 2025 <Program> 5x5 Advanced \| LiftVault.com` | → `Feb 2025 Workout Log` |
