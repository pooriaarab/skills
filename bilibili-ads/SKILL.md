---
name: bilibili-ads
description: "Set up Bilibili Ads tracking as a regional or CTV adapter — official token or partner access, client tag or app measurement, server conversion shape, audience limits, reporting verification, and explicit unverified boundaries. Use when evaluating Bilibili Ads, planning a regional campaign, or deciding whether a production integration exists."
---
...
No stable public Bilibili Ads conversion API contract was verified. Confirm advertiser API access and pixel details with the official platform.

Official source: [Bilibili Ads documentation](https://open.bilibili.com/).
...
measurement source at [Bilibili Ads](https://open.bilibili.com/). Use only code copied from the vendor
console. Store the public ID as `BILIBILI_ADS_PIXEL_ID` or `BILIBILI_ADS_TAG_ID` after the vendor
confirms its name.
...
source: [Bilibili Ads](https://open.bilibili.com/).
...
1. Open the official Bilibili Ads developer or advertiser site: [Bilibili Ads](https://open.bilibili.com/).
...
4. Store the approved credential as `BILIBILI_ADS_TOKEN`. Keep it server-side.
...
```bash
curl -sS 'https://open.bilibili.com/' -H "Authorization: Bearer $BILIBILI_ADS_TOKEN"
```
...
fields, minimum usable size, lookalike expansion, and eligibility at [Bilibili Ads](https://open.bilibili.com/).
...
- Make `BILIBILI_ADS_PIXEL_ID` and `BILIBILI_ADS_TOKEN` absent-safe no-ops.
...
Load code only from the vendor's official HTTPS origin. Keep `BILIBILI_ADS_TOKEN`
