---
name: weibo-ads
description: "Set up Weibo Ads tracking as a regional or CTV adapter — official token or partner access, client tag or app measurement, server conversion shape, audience limits, reporting verification, and explicit unverified boundaries. Use when evaluating Weibo Ads, planning a regional campaign, or deciding whether a production integration exists."
---
...
No stable public Weibo Ads conversion API contract was verified. Use the official Weibo open platform and account team for access.

Official source: [Weibo Ads documentation](https://open.weibo.com/).
...
measurement source at [Weibo Ads](https://open.weibo.com/). Use only code copied from the vendor
console. Store the public ID as `WEIBO_ADS_PIXEL_ID` or `WEIBO_ADS_TAG_ID` after the vendor
confirms its name.
...
source: [Weibo Ads](https://open.weibo.com/).
...
1. Open the official Weibo Ads developer or advertiser site: [Weibo Ads](https://open.weibo.com/).
...
4. Store the approved credential as `WEIBO_ADS_TOKEN`. Keep it server-side.
...
```bash
curl -sS 'https://open.weibo.com/' -H "Authorization: Bearer $WEIBO_ADS_TOKEN"
```
...
fields, minimum usable size, lookalike expansion, and eligibility at [Weibo Ads](https://open.weibo.com/).
...
- Make `WEIBO_ADS_PIXEL_ID` and `WEIBO_ADS_TOKEN` absent-safe no-ops.
...
Load code only from the vendor's official HTTPS origin. Keep `WEIBO_ADS_TOKEN`
