---
name: amazon-ads
description: "Amazon Ads tracking is product-specific and public API coverage differs between Sponsored Ads, Attribution, and DSP. Use when setting up Amazon Ads, debugging conversion tracking, building retargeting audiences, or validating a small paid test."
---
...
> ⚠ UNVERIFIED — confirm the current client tag URL, init call, pixel ID field, and purchase event shape at [Amazon Ads tracking documentation](https://advertising.amazon.com/attribution).

Store the identifier as `AMAZON_ADS_PIXEL_ID` or `AMAZON_ADS_TAG_ID`, depending on the platform's official naming.

## Server-side conversion API

The platform has an official API surface, but the request shape depends on account product and access level.

> ⚠ UNVERIFIED — confirm the current POST endpoint, auth header, timestamp unit, deduplication field, and JSON payload at [Amazon Ads API documentation](https://advertising.amazon.com/API/docs/en-us).
...
1. Open the official console for [Amazon Ads](https://advertising.amazon.com/attribution).
...
4. Store it as `AMAZON_ADS_TOKEN`. Keep it server-side.

> ⚠ UNVERIFIED — confirm the current click path, OAuth scopes, expiry, and credential type at [Amazon Ads official API docs](https://advertising.amazon.com/API/docs/en-us).

Token validation:

```bash
curl -sS -H "Authorization: Bearer $AMAZON_ADS_TOKEN" 'https://advertising.amazon.com/API/docs/en-us'
```
...
> ⚠ UNVERIFIED — confirm hashed-email fields, minimum list size, match-rate rules, lookalike or expansion availability, and failure modes at [Amazon Ads audience documentation](https://advertising.amazon.com/attribution).
...
- Make an absent `AMAZON_ADS_PIXEL_ID` or `AMAZON_ADS_TOKEN` a logged no-op.
...
- ⚠ UNVERIFIED — confirm current bid floors, learning thresholds, minimum budgets, and default placements at [Amazon Ads campaign documentation](https://advertising.amazon.com/API/docs/en-us).

## Verification

Use the official test-event view or reporting API for Amazon Ads. Reconcile its conversions with the payment provider's succeeded charges.

> ⚠ UNVERIFIED — confirm the current test-event endpoint, reporting query, delay, and status fields at [Amazon Ads official API docs](https://advertising.amazon.com/API/docs/en-us).
...
Load client code only from the vendor's official HTTPS origin. Keep `AMAZON_ADS_TOKEN`
