...
description: "Set up Xiaohongshu Ads tracking as a regional or CTV adapter — official token or partner access, client tag or app measurement, server conversion shape, audience limits, reporting verification, and explicit unverified boundaries. Use when evaluating Xiaohongshu Ads, planning a regional campaign, or deciding whether a production integration exists."
---
...
No stable public Xiaohongshu Ads conversion API contract was verified. Confirm advertiser access and tracking terms in the official platform.

Official source: [Xiaohongshu Ads documentation](https://open.xiaohongshu.com/).
...
measurement source at [Xiaohongshu Ads](https://open.xiaohongshu.com/). Use only code copied from the vendor
console. Store the public ID as `XIAOHONGSHU_ADS_PIXEL_ID` or `XIAOHONGSHU_ADS_TAG_ID` after the vendor
confirms its name.
...
source: [Xiaohongshu Ads](https://open.xiaohongshu.com/).
...
1. Open the official Xiaohongshu Ads developer or advertiser site: [Xiaohongshu Ads](https://open.xiaohongshu.com/).
...
4. Store the approved credential as `XIAOHONGSHU_ADS_TOKEN`. Keep it server-side.
...
```bash
curl -sS 'https://open.xiaohongshu.com/' -H "Authorization: Bearer $XIAOHONGSHU_ADS_TOKEN"
```
...
fields, minimum usable size, lookalike expansion, and eligibility at [Xiaohongshu Ads](https://open.xiaohongshu.com/).
...
- Make `XIAOHONGSHU_ADS_PIXEL_ID` and `XIAOHONGSHU_ADS_TOKEN` absent-safe no-ops.
...
Load code only from the vendor's official HTTPS origin. Keep `XIAOHONGSHU_ADS_TOKEN`
