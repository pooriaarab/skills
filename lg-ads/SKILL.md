---
name: lg-ads
description: "Set up LG Ads tracking as a regional or CTV adapter — official token or partner access, client tag or app measurement, server conversion shape, audience limits, reporting verification, and explicit unverified boundaries. Use when evaluating LG Ads, planning a regional campaign, or deciding whether a production integration exists."
---
...
No stable public LG Ads conversion API contract was verified. Request the current managed-CTV measurement guide from LG Ads.

Official source: [LG Ads documentation](https://lgads.tv/).
...
measurement source at [LG Ads](https://lgads.tv/). Use only code copied from the vendor
console. Store the public ID as `LG_ADS_PIXEL_ID` or `LG_ADS_TAG_ID` after the vendor
confirms its name.
...
1. Open the official LG Ads developer or advertiser site: [LG Ads](https://lgads.tv/).
...
4. Store the approved credential as `LG_ADS_TOKEN`. Keep it server-side.
...
```bash
curl -sS 'https://lgads.tv/' -H "Authorization: Bearer $LG_ADS_TOKEN"
```
...
fields, minimum usable size, lookalike expansion, and eligibility at [LG Ads](https://lgads.tv/).
...
- Make `LG_ADS_PIXEL_ID` and `LG_ADS_TOKEN` absent-safe no-ops.
...
Load code only from the vendor's official HTTPS origin. Keep `LG_ADS_TOKEN`
