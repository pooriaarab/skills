---
name: kakao-ads
description: "Set up Kakao Ads tracking as a regional or CTV adapter — official token or partner access, client tag or app measurement, server conversion shape, audience limits, reporting verification, and explicit unverified boundaries. Use when evaluating Kakao Ads, planning a regional campaign, or deciding whether a production integration exists."
---
...
Kakao Moment documents tracker and SDK resources, business-token authentication, ad account APIs, and audience endpoints.

Official source: [Kakao Ads documentation](https://developers.kakao.com/docs/en/kakaomoment/reference).
...
Kakao Moment exposes Pixel & SDK tracker resources. The API can list authorized trackers with `GET https://apis.moment.kakao.com/openapi/v4/adAccounts/trackers` using a business token. [Official reference](https://developers.kakao.com/docs/en/kakaomoment/ad-account).
...
source: [Kakao Ads](https://developers.kakao.com/docs/en/kakaomoment/reference).
...
1. Open the official Kakao Ads developer or advertiser site: [Kakao Ads](https://developers.kakao.com/docs/en/kakaomoment/reference).
...
4. Store the approved credential as `KAKAO_ADS_TOKEN`. Keep it server-side.
...
```bash
curl -sS 'https://developers.kakao.com/docs/en/kakaomoment/reference' -H "Authorization: Bearer $KAKAO_ADS_TOKEN"
```
...
Kakao Moment documents audience management and tracker linkage. Customer-file fields, hashing, and list floors must be checked in the current Korean/English reference.
...
- Make `KAKAO_ADS_PIXEL_ID` and `KAKAO_ADS_TOKEN` absent-safe no-ops.
...
Load code only from the vendor's official HTTPS origin. Keep `KAKAO_ADS_TOKEN`
...
- https://developers.kakao.com/docs/en/kakaomoment/reference
