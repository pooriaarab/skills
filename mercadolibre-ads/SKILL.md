...
description: "Set up Mercado Libre Ads tracking as a regional or CTV adapter — official token or partner access, client tag or app measurement, server conversion shape, audience limits, reporting verification, and explicit unverified boundaries. Use when evaluating Mercado Libre Ads, planning a regional campaign, or deciding whether a production integration exists."
---
...
Mercado Libre provides an official developer portal. Confirm Mercado Ads tracking, sponsored product, and audience APIs for the marketplace country.

Official source: [Mercado Libre Ads documentation](https://developers.mercadolibre.com/).
...
measurement source at [Mercado Libre Ads](https://developers.mercadolibre.com/). Use only code copied from the vendor
console. Store the public ID as `MERCADOLIBRE_ADS_PIXEL_ID` or `MERCADOLIBRE_ADS_TAG_ID` after the vendor
confirms its name.
...
source: [Mercado Libre Ads](https://developers.mercadolibre.com/).
...
1. Open the official Mercado Libre Ads developer or advertiser site: [Mercado Libre Ads](https://developers.mercadolibre.com/).
...
4. Store the approved credential as `MERCADOLIBRE_ADS_TOKEN`. Keep it server-side.
...
```bash
curl -sS 'https://developers.mercadolibre.com/' -H "Authorization: Bearer $MERCADOLIBRE_ADS_TOKEN"
```
...
fields, minimum usable size, lookalike expansion, and eligibility at [Mercado Libre Ads](https://developers.mercadolibre.com/).
...
- Make `MERCADOLIBRE_ADS_PIXEL_ID` and `MERCADOLIBRE_ADS_TOKEN` absent-safe no-ops.
...
Load code only from the vendor's official HTTPS origin. Keep `MERCADOLIBRE_ADS_TOKEN`
