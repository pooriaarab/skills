...
**Motion.** `cubic-bezier(0.22, 1, 0.36, 1)`, `380ms`. On navigation the incoming sheet translates up 24px into place. **Its opacity never animates.** The sheet is at its resting 94% from the first frame and only `transform: translateY()` runs, so the outgoing sheet is still sliding out underneath it and for roughly 180ms in the middle both pages are legible through one another — you read the outgoing headline through the incoming one. The overlap comes from the resting translucency of the material, not from a crossfade, which is why it costs nothing: a crawler, a print, a screenshot or a reader who lands mid-navigation always gets a fully painted sheet. That overlap is the entire animation; nothing else moves, ever. Under `prefers-reduced-motion` the sheet is placed with no translate.
...
**The stack is the progress bar.** Slide *n* carries `min(n, 5)` sheets, so slide 5 carries five sheets and therefore **four** overlaps, not three. Each overlap multiplies the field by 0.94, and 0.94⁴ = 0.78, so the deepest corner of slide 5 is roughly **22% darker** than a single sheet. The reader sees how deep they are without a counter. Cap at 5 sheets. At slide 6 reset to a single sheet and mark the reset with a Courier Prime caps marker — `DRAFT 2` — at 28px in the top-left. A ten-slide carousel is therefore two stacks of five, and the reset is the chapter break.
...
**Pairs with [`../brand-dusk/SKILL.md`](../brand-dusk/SKILL.md).** Both are warm, unhurried and native to work with a history. vellum is the stack of drafts; dusk is the reflection written on top of it. Vermilion `#a8331f` and ember `#f2a25c` share a warm quadrant.

**Sits beside [`../brand-manuscript/SKILL.md`](../brand-manuscript/SKILL.md), and the two are easy to confuse.** Both are paper and both are warm; the distinction is structural. **Manuscript is a bound book: one leaf at a time, a rubric drop cap, a fixed text block, an authority already settled. vellum is loose translucent sheets: a stack, visible earlier drafts, no drop cap, nothing settled.** Content with a final version goes to manuscript; content with versions goes to vellum. Never run both in one kit — the reader sees one paper direction executed inconsistently.

**Clashes with [`../brand-swiss/SKILL.md`](../brand-swiss/SKILL.md).** Swiss is one flat plane with no material at all. Next to it vellum's alpha reads as dirt on the screen rather than as substance.

**Clashes with [`../brand-arcade/SKILL.md`](../brand-arcade/SKILL.md).** Arcade is instant and lit from inside; vellum is slow and lit from outside. No shared surface.
