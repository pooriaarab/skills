...
3. **Landing page (recommend).** Healthy CTR but conversions stuck at zero at real volume ⇒ the LP/offer is the suspect, not the ads. Recommend an LP/CTA variant test. (See "conversion is the lever" below.)
...
5. **Feature-utilization audit (recommend, one line/cycle).** Name the single highest-value platform feature you're leaving unused — most often **conversion-optimized bidding**, which is the biggest lever but is *gated on real conversion tracking* being live. Surface it; don't switch to it until the tracking feeds it.
...
- **Lower-friction landing beats a form.** Capturing the visitor's own link and forwarding it (prefilling the product's create flow) tends to convert better than an email gate — fewer steps before value.
