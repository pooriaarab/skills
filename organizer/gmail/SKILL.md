...
**Path B — Gmail API directly (use when the user already has OAuth creds wired up, e.g. from another project like <work-repo>)**
...
**Mozilla / Workspace OAuth caveat:** if the active account is `<work-account>`, the Workspace admin may block third-party apps from acquiring `gmail.modify`. If `gws auth login` fails at the consent screen with a "blocked by your administrator" message, switch to `personal` and run there first. Do not file an IT ticket without checking this skill ran cleanly on the personal account first — the skill itself is what they'll want to vet.
