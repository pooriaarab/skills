...
3. Replace the example protected-CRUD resource with the new idea's actual first domain object, keeping the same auth/D1/migration pattern rather than inventing a new one.
4. From here, the idea's real design/build work is its own process (brainstorm the domain model, plan, implement, test) — this skill's job ends at "you have a running scaffold with auth and one real resource," not at "the product is built."
5. Once there's real, working product code, move to `open-source-repo-prep` if it's going public, then `launch-video-generation` + `social-launch-post` for the announcement. See `ship-a-product` for how these chain together.
