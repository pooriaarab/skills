---
name: baidu-ads
description: "Set up Baidu Ads tracking as a regional or CTV adapter — official token or partner access, client tag or app measurement, server conversion shape, audience limits, reporting verification, and explicit unverified boundaries. Use when evaluating Baidu Ads, planning a regional campaign, or deciding whether a production integration exists."
---
...
Baidu's Commercial Developer Center documents OAuth 2.0, conversion tracking, web JavaScript code, and API callbacks. Confirm Chinese account eligibility and current API operation names.

Official source: [Baidu Ads documentation](https://dev2.baidu.com/content).
...
measurement source at [Baidu Ads](https://dev2.baidu.com/content). Use only code copied from the vendor
console. Store the public ID as `BAIDU_ADS_PIXEL_ID` or `BAIDU_ADS_TAG_ID` after the vendor
confirms its name.
...
source: [Baidu Ads](https://dev2.baidu.com/content).
...
1. Open the official Baidu Ads developer or advertiser site: [Baidu Ads](https://dev2.baidu.com/content).
...
4. Store the approved credential as `BAIDU_ADS_TOKEN`. Keep it server-side.
...
```bash
curl -sS 'https://dev2.baidu.com/content' -H "Authorization: Bearer $BAIDU_ADS_TOKEN"
```
...
fields, minimum usable size, lookalike expansion, and eligibility at [Baidu Ads](https://dev2.baidu.com/content).
...
- Make `BAIDU_ADS_PIXEL_ID` and `BAIDU_ADS_TOKEN` absent-safe no-ops.
...
Load code only from the vendor's official HTTPS origin. Keep `BAIDU_ADS_TOKEN`
