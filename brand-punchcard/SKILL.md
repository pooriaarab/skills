...
**Pairs with [terminal](../brand-terminal/SKILL.md).** Terminal reports the session as it runs; punchcard files the record afterwards. Green-on-black beside manila is a strong contrast with no hue conflict, and the two split the timeline cleanly between now and then.
...
**Note on the mono directions:** punchcard is **not** one of them, and this matters when picking a kit. [terminal](../brand-terminal/SKILL.md), [spec](../brand-spec/SKILL.md) and [oscilloscope](../brand-oscilloscope/SKILL.md) each make a different argument for monospace. Punchcard's prose is Archivo, and Courier Prime appears only on the ruler, the IDs and the dates. That makes punchcard safe to pair with any one of the three, where those three are not safe to pair with each other.
