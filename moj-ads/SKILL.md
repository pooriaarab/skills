---
name: moj-ads
description: "Set up Moj Ads tracking as a regional or CTV adapter — official token or partner access, client tag or app measurement, server conversion shape, audience limits, reporting verification, and explicit unverified boundaries. Use when evaluating Moj Ads, planning a regional campaign, or deciding whether a production integration exists."
---
...
No stable public Moj Ads conversion API contract was verified. Confirm advertiser access, tag, and measurement terms with Moj.

Official source: [Moj Ads documentation](https://mojapp.in/).
...
measurement source at [Moj Ads](https://mojapp.in/). Use only code copied from the vendor
console. Store the public ID as `MOJ_ADS_PIXEL_ID` or `MOJ_ADS_TAG_ID` after the vendor
confirms its name.
...
source: [Moj Ads](https://mojapp.in/).
...
1. Open the official Moj Ads developer or advertiser site: [Moj Ads](https://mojapp.in/).
...
4. Store the approved credential as `MOJ_ADS_TOKEN`. Keep it server-side.
...
```bash
curl -sS 'https://mojapp.in/' -H "Authorization: Bearer $MOJ_ADS_TOKEN"
```
...
fields, minimum usable size, lookalike expansion, and eligibility at [Moj Ads](https://mojapp.in/).
...
- Make `MOJ_ADS_PIXEL_ID` and `MOJ_ADS_TOKEN` absent-safe no-ops.
...
Load code only from the vendor's official HTTPS origin. Keep `MOJ_ADS_TOKEN`
