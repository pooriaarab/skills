...
**Pairs with [`../vellum/SKILL.md`](../vellum/SKILL.md).** Both are warm, unhurried, and native to work with a history. dusk is the reflection at the end of the day; vellum is the stack of drafts that produced it. The ember `#f2a25c` and the vermilion `#a8331f` sit in the same warm quadrant, so a kit that runs dusk for essays and vellum for changelogs holds together.

**Pairs with [`../broadsheet/SKILL.md`](../broadsheet/SKILL.md).** Shared long-form register, shared single column. Broadsheet takes the reported piece; dusk takes the personal one.

**Clashes with [`../stadium/SKILL.md`](../stadium/SKILL.md).** Stadium is the present tense at volume; dusk is the past tense at a murmur. Run both on one account in one week and the reader cannot tell whether anything is currently happening.

**Clashes with [`../arcade/SKILL.md`](../arcade/SKILL.md).** Arcade is now, lit and loud; dusk is after, dim and slow. No surface is right for both.
