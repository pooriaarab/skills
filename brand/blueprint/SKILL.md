...
**Motion — the one authored moment.** Ease `cubic-bezier(0.33, 0, 0.15, 1)`, duration **880ms**. The dimension line under the headline draws itself in one pass, in drafting order: the left arrowhead snaps in at full opacity, the 1px rule extends rightward at constant speed across the headline's exact width, the right arrowhead snaps in on arrival, and only then does the measurement fade up into the gap the rule left open at its midpoint. Once per page load. **Nothing else on the page moves, ever.** Under `prefers-reduced-motion: reduce`, render the finished dimension line and skip the draw.
