...
The original scripts below were hardcoded to one work repo (<work-repo>, Mozilla GCP). They are now generalized into a **work/personal split** so any private repo offloads to its OWN GCP project — personal repos must NEVER run on Mozilla infra (same confidentiality boundary as `claude-work`/`claude-personal`).
...
The sections below document the original single-repo baked-image design (still how the WORK/<work-repo> side runs). The generalized personal side above trades the bake for a raw base + runtime install.
