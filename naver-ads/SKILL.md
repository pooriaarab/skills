---
name: naver-ads
description: "Set up Naver Ads tracking as a regional or CTV adapter — official token or partner access, client tag or app measurement, server conversion shape, audience limits, reporting verification, and explicit unverified boundaries. Use when evaluating Naver Ads, planning a regional campaign, or deciding whether a production integration exists."
---
...
Naver provides official developer documentation and conversion-tracking tools for Naver advertising. Confirm the current Search Ads API access and Korean account requirements.

Official source: [Naver Ads documentation](https://developers.naver.com/).
...
measurement source at [Naver Ads](https://developers.naver.com/). Use only code copied from the vendor
console. Store the public ID as `NAVER_ADS_PIXEL_ID` or `NAVER_ADS_TAG_ID` after the vendor
confirms its name.
...
source: [Naver Ads](https://developers.naver.com/).
...
1. Open the official Naver Ads developer or advertiser site: [Naver Ads](https://developers.naver.com/).
...
4. Store the approved credential as `NAVER_ADS_TOKEN`. Keep it server-side.
...
```bash
curl -sS 'https://developers.naver.com/' -H "Authorization: Bearer $NAVER_ADS_TOKEN"
```
...
fields, minimum usable size, lookalike expansion, and eligibility at [Naver Ads](https://developers.naver.com/).
...
- Make `NAVER_ADS_PIXEL_ID` and `NAVER_ADS_TOKEN` absent-safe no-ops.
...
Load code only from the vendor's official HTTPS origin. Keep `NAVER_ADS_TOKEN`
