---
name: twitch-ads
description: "Set up Twitch Ads tracking as a regional or CTV adapter — official token or partner access, client tag or app measurement, server conversion shape, audience limits, reporting verification, and explicit unverified boundaries. Use when evaluating Twitch Ads, planning a regional campaign, or deciding whether a production integration exists."
---
...
Twitch's public API documents broadcaster ad controls and OAuth. Advertiser conversion measurement is a separate product surface and remains unverified here.

Official source: [Twitch Ads documentation](https://dev.twitch.tv/docs/api/reference).
...
measurement source at [Twitch Ads](https://dev.twitch.tv/docs/api/reference). Use only code copied from the vendor
console. Store the public ID as `TWITCH_ADS_PIXEL_ID` or `TWITCH_ADS_TAG_ID` after the vendor
confirms its name.
...
source: [Twitch Ads](https://dev.twitch.tv/docs/api/reference).
...
1. Open the official Twitch Ads developer or advertiser site: [Twitch Ads](https://dev.twitch.tv/docs/api/reference).
...
4. Store the approved credential as `TWITCH_ADS_TOKEN`. Keep it server-side.
...
```bash
curl -sS 'https://dev.twitch.tv/docs/api/reference' -H "Authorization: Bearer $TWITCH_ADS_TOKEN"
```
...
fields, minimum usable size, lookalike expansion, and eligibility at [Twitch Ads](https://dev.twitch.tv/docs/api/reference).
...
- Make `TWITCH_ADS_PIXEL_ID` and `TWITCH_ADS_TOKEN` absent-safe no-ops.
...
Load code only from the vendor's official HTTPS origin. Keep `TWITCH_ADS_TOKEN`
