---
name: vk-ads
description: "Set up VK Ads tracking as a regional or CTV adapter — official token or partner access, client tag or app measurement, server conversion shape, audience limits, reporting verification, and explicit unverified boundaries. Use when evaluating VK Ads, planning a regional campaign, or deciding whether a production integration exists."
---
...
VK Ads API details and tracking products vary by region and account. Confirm official access and current conversion event docs.

Official source: [VK Ads documentation](https://dev.vk.com/).
...
measurement source at [VK Ads](https://dev.vk.com/). Use only code copied from the vendor
console. Store the public ID as `VK_ADS_PIXEL_ID` or `VK_ADS_TAG_ID` after the vendor
confirms its name.
...
source: [VK Ads](https://dev.vk.com/).
...
1. Open the official VK Ads developer or advertiser site: [VK Ads](https://dev.vk.com/).
...
4. Store the approved credential as `VK_ADS_TOKEN`. Keep it server-side.
...
```bash
curl -sS 'https://dev.vk.com/' -H "Authorization: Bearer $VK_ADS_TOKEN"
```
...
fields, minimum usable size, lookalike expansion, and eligibility at [VK Ads](https://dev.vk.com/).
...
- Make `VK_ADS_PIXEL_ID` and `VK_ADS_TOKEN` absent-safe no-ops.
...
Load code only from the vendor's official HTTPS origin. Keep `VK_ADS_TOKEN`
