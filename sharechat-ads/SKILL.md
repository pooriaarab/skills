---
name: sharechat-ads
description: "Set up ShareChat Ads tracking as a regional or CTV adapter — official token or partner access, client tag or app measurement, server conversion shape, audience limits, reporting verification, and explicit unverified boundaries. Use when evaluating ShareChat Ads, planning a regional campaign, or deciding whether a production integration exists."
---
...
No stable public ShareChat Ads conversion API contract was verified. Request the current advertiser integration guide from ShareChat.

Official source: [ShareChat Ads documentation](https://sharechat.com/).
...
measurement source at [ShareChat Ads](https://sharechat.com/). Use only code copied from the vendor
console. Store the public ID as `SHARECHAT_ADS_PIXEL_ID` or `SHARECHAT_ADS_TAG_ID` after the vendor
confirms its name.
...
source: [ShareChat Ads](https://sharechat.com/).
...
1. Open the official ShareChat Ads developer or advertiser site: [ShareChat Ads](https://sharechat.com/).
...
4. Store the approved credential as `SHARECHAT_ADS_TOKEN`. Keep it server-side.
...
```bash
curl -sS 'https://sharechat.com/' -H "Authorization: Bearer $SHARECHAT_ADS_TOKEN"
```
...
fields, minimum usable size, lookalike expansion, and eligibility at [ShareChat Ads](https://sharechat.com/).
...
- Make `SHARECHAT_ADS_PIXEL_ID` and `SHARECHAT_ADS_TOKEN` absent-safe no-ops.
...
Load code only from the vendor's official HTTPS origin. Keep `SHARECHAT_ADS_TOKEN`
