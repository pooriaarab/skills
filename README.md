...
| [ci-speed-diagnosis](ci-speed-diagnosis/SKILL.md) | Find out why the CI optimisations you already have do nothing, before adding more. The measure-first order (latency vs machine seconds, then the critical path, then cache health), the checklist of defects that fail silently behind a green check (a secret holding an empty value, a task runner stripping `TURBO_*` from child processes, a `.next/**` output glob that sweeps the incremental cache into a 1.2 GB artifact whose upload fails, ref-scoped duplicate caches filling the quota, `restore-keys` with no lockfile-independent fallback, Prettier's mtime cache strategy that `actions/checkout` defeats, a buffering cache server that caps uploads), the bug shape that recurs most — one host's mitigation applied on every host — and the measurement discipline that keeps the numbers honest. |
