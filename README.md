...
| [pre-public-repo-audit](pre-public-repo-audit/SKILL.md) | Audit a repo before you flip it public, and know why a clean secret-scanner run is not the answer. The five sweeps that catch what `gitleaks` structurally cannot — bare high-entropy keys with no vendor rule, absolute laptop paths (which usually name a private repo too), PII in files and commit metadata, tracked build artifacts — plus the order of operations (scan, fix, rotate, then flip) and why making a leaked repo private again rotates nothing. Runs before `open-source-repo-prep`. |
