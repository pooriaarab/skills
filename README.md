...
| [dell-windows-host](dell-windows-host/SKILL.md) | Operate a personal always-on Windows + WSL2 desktop as repository-scoped GitHub Actions runners: register one runner service per private repo, pick which jobs are safe to move off hosted runners, read a queued job as a WSL restart, and know exactly why a synced Crabbox run on a private repo still fails. |
