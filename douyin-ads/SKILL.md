---
name: douyin-ads
description: "Set up Douyin Ads tracking as a regional or CTV adapter — official token or partner access, client tag or app measurement, server conversion shape, audience limits, reporting verification, and explicit unverified boundaries. Use when evaluating Douyin Ads, planning a regional campaign, or deciding whether a production integration exists."
---
...
Ocean Engine provides API tools, OAuth2 access, and conversion tracking for Douyin advertising. Confirm the current event API and account scope.

Official source: [Douyin Ads documentation](https://open.oceanengine.com/tools).
...
measurement source at [Douyin Ads](https://open.oceanengine.com/tools). Use only code copied from the vendor
console. Store the public ID as `DOUYIN_ADS_PIXEL_ID` or `DOUYIN_ADS_TAG_ID` after the vendor
confirms its name.
...
source: [Douyin Ads](https://open.oceanengine.com/tools).
...
1. Open the official Douyin Ads developer or advertiser site: [Douyin Ads](https://open.oceanengine.com/tools).
...
4. Store the approved credential as `DOUYIN_ADS_TOKEN`. Keep it server-side.
...
```bash
curl -sS 'https://open.oceanengine.com/tools' -H "Authorization: Bearer $DOUYIN_ADS_TOKEN"
```
...
fields, minimum usable size, lookalike expansion, and eligibility at [Douyin Ads](https://open.oceanengine.com/tools).
...
- Make `DOUYIN_ADS_PIXEL_ID` and `DOUYIN_ADS_TOKEN` absent-safe no-ops.
...
Load code only from the vendor's official HTTPS origin. Keep `DOUYIN_ADS_TOKEN`
...
- https://open.oceanengine.com/tools
