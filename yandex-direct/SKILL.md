---
name: yandex-direct
description: "Set up Yandex Direct tracking as a regional or CTV adapter — official token or partner access, client tag or app measurement, server conversion shape, audience limits, reporting verification, and explicit unverified boundaries. Use when evaluating Yandex Direct, planning a regional campaign, or deciding whether a production integration exists."
---
...
Yandex Direct provides an official API reference. Confirm the current conversion and audience endpoints for the account region.

Official source: [Yandex Direct documentation](https://yandex.com/dev/direct/).
...
measurement source at [Yandex Direct](https://yandex.com/dev/direct/). Use only code copied from the vendor
console. Store the public ID as `YANDEX_DIRECT_PIXEL_ID` or `YANDEX_DIRECT_TAG_ID` after the vendor
confirms its name.
...
source: [Yandex Direct](https://yandex.com/dev/direct/).
...
1. Open the official Yandex Direct developer or advertiser site: [Yandex Direct](https://yandex.com/dev/direct/).
...
4. Store the approved credential as `YANDEX_DIRECT_TOKEN`. Keep it server-side.
...
```bash
curl -sS 'https://yandex.com/dev/direct/' -H "Authorization: Bearer $YANDEX_DIRECT_TOKEN"
```
...
fields, minimum usable size, lookalike expansion, and eligibility at [Yandex Direct](https://yandex.com/dev/direct/).
...
- Make `YANDEX_DIRECT_PIXEL_ID` and `YANDEX_DIRECT_TOKEN` absent-safe no-ops.
...
Load code only from the vendor's official HTTPS origin. Keep `YANDEX_DIRECT_TOKEN`
