---
name: shopee-ads
description: "Set up Shopee Ads tracking as a regional or CTV adapter — official token or partner access, client tag or app measurement, server conversion shape, audience limits, reporting verification, and explicit unverified boundaries. Use when evaluating Shopee Ads, planning a regional campaign, or deciding whether a production integration exists."
---
...
Shopee Open Platform documents merchant APIs, but a public product-agnostic Shopee Ads conversion API was not verified.

Official source: [Shopee Ads documentation](https://open.shopee.com/).
...
measurement source at [Shopee Ads](https://open.shopee.com/). Use only code copied from the vendor
console. Store the public ID as `SHOPEE_ADS_PIXEL_ID` or `SHOPEE_ADS_TAG_ID` after the vendor
confirms its name.
...
source: [Shopee Ads](https://open.shopee.com/).
...
1. Open the official Shopee Ads developer or advertiser site: [Shopee Ads](https://open.shopee.com/).
...
4. Store the approved credential as `SHOPEE_ADS_TOKEN`. Keep it server-side.
...
```bash
curl -sS 'https://open.shopee.com/' -H "Authorization: Bearer $SHOPEE_ADS_TOKEN"
```
...
fields, minimum usable size, lookalike expansion, and eligibility at [Shopee Ads](https://open.shopee.com/).
...
- Make `SHOPEE_ADS_PIXEL_ID` and `SHOPEE_ADS_TOKEN` absent-safe no-ops.
...
Load code only from the vendor's official HTTPS origin. Keep `SHOPEE_ADS_TOKEN`
