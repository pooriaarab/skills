...
- **The scenarios are the real work, and NOT automatable.** The Review form has **one field PER MODULE** — a separate Make scenario URL (`https://us1.make.com/{orgId}/scenarios/{id}/edit`) for **every** module, each showing a **successful run** of that module — plus one more field for a scenario that deliberately triggers an **API error** (e.g. a getter with a bogus id). For a 32-module app that's 33 scenarios you build and run by hand. The SDK Apps API builds the app but **cannot** produce these: the Scenarios API can create empty scenarios but can't make them meaningfully pass, and modules with side effects (create/publish/delete) act for real — so budget genuine manual QA time. Also toggle **every module to "visible"** first (the form checks it). Run the scenarios right before requesting review — execution logs expire. Publishing is **permanent (no unpublish)**, so only Publish when you're ready to do all this.
