---
name: kuaishou-ads
description: "Set up Kuaishou Ads tracking as a regional or CTV adapter — official token or partner access, client tag or app measurement, server conversion shape, audience limits, reporting verification, and explicit unverified boundaries. Use when evaluating Kuaishou Ads, planning a regional campaign, or deciding whether a production integration exists."
---
...
Kuaishou documents a Marketing API platform and conversion callback flows. One official mini-program guide documents HTTPS GET activation reporting with callback, event_type, and 13-digit event_time.

Official source: [Kuaishou Ads documentation](https://developers.e.kuaishou.com/).
...
measurement source at [Kuaishou Ads](https://developers.e.kuaishou.com/). Use only code copied from the vendor
console. Store the public ID as `KUAISHOU_ADS_PIXEL_ID` or `KUAISHOU_ADS_TAG_ID` after the vendor
confirms its name.

## Server-side conversion API

The official Kuaishou mini-program advertising guide documents:

```text
GET https://ad.partner.gifshow.com/track/activate?event_type=1&event_time=<13-digit-ms>&callback=<callback>
```

The response uses `{"result":1}` for a successful report. This is a callback
flow for the documented mini-program scenario, not a universal web CAPI. [Official
guide](https://open.kuaishou.com/miniGameDocs/operation/DSP/DSP-IAA).

## Get a token and validate it

1. Open the official Kuaishou Ads developer or advertiser site: [Kuaishou Ads](https://developers.e.kuaishou.com/).
...
4. Store the approved credential as `KUAISHOU_ADS_TOKEN`. Keep it server-side.
...
```bash
curl -sS 'https://developers.e.kuaishou.com/' -H "Authorization: Bearer $KUAISHOU_ADS_TOKEN"
```
...
fields, minimum usable size, lookalike expansion, and eligibility at [Kuaishou Ads](https://developers.e.kuaishou.com/).
...
- Make `KUAISHOU_ADS_PIXEL_ID` and `KUAISHOU_ADS_TOKEN` absent-safe no-ops.
...
Load code only from the vendor's official HTTPS origin. Keep `KUAISHOU_ADS_TOKEN`
...
- https://developers.e.kuaishou.com/
